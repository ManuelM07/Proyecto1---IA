...
def crear_hijos(nodo_padre):
    ...
    for i in range(len(operadores)-1, -1, -1): # [a, b, c] # e -> a

        if (operadores[i] == "arriba" and arriba > -1 and arriba != 1): #verifica que se puede mover (no sale de la matriz y no hay un muro).
            if (nodo_padre.operador == "abajo" and (nodo_padre.nave != nodo_padre.nodo_padre.nave or nodo_padre.nodo_padre.item_encontrado) or # verifica que se puede devolver
                (nodo_padre.operador != "abajo" )): # O verifica que no se regrese
                
                matriz_copia = restringir_camino(matriz_copia, x, y).copy() 
                matriz_copia = validar_cambio_matriz(nodo_padre, matriz_copia, nave_hijo, x, y).copy() 
            ...
            if (nodo_padre.operador == "arriba" and (nodo_padre.nave != nodo_padre.nodo_padre.nave or nodo_padre.nodo_padre.item_encontrado) or # se verifica que se puede devolver
            ...
            if (nodo_padre.operador == "derecha" and (nodo_padre.nave != nodo_padre.nodo_padre.nave or nodo_padre.nodo_padre.item_encontrado) or # se verifica que se puede devolver
            ...
            if (nodo_padre.operador == "izquierda" and (nodo_padre.nave != nodo_padre.nodo_padre.nave or nodo_padre.nodo_padre.item_encontrado) or # se verifica que se puede devolver
...
def validar_cambio_matriz(nodo_padre, matriz, nave_hijo, x, y):
    try:
...
def restringir_camino(matriz, x, y):
    if matriz[x][y] == 6 or matriz[x][y] == -6:
        ...
        matriz[x][y] = -2
    #elif matriz[x][y] == 5 or matriz[x][y] == -5:
    #    matriz[x][y] = -5
    else:
        matriz[x][y] = -1
    return matriz 